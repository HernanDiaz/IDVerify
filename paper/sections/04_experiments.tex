...
\begin{table*}[htbp]
  \centering
  \caption{Comparison with DeepID 2025 Challenge participants.
           $\dagger$: challenge systems evaluated on the official test set;
           DocVerify evaluated on an internal 15\% holdout (see text).
           ``—'' indicates the system did not participate in that track.}
  \label{tab:challenge}
  \begin{tabular}{lcccc}
    \toprule
    System & F1 det (thr=0.5) & F1 loc (per-image) & Training Data \\
    \midrule
    Sunlight (1st)$^\dagger$        & 0.991 & 0.784 & 60K+ external images \\
    Incode (2nd)$^\dagger$          & 0.868 & —     & 100K proprietary images \\
    AG / EdgeDoc (3rd)$^\dagger$    & 0.958 & 0.686 & FantasyID only \\
    UAM-Biometrics (4th)$^\dagger$  & 0.712 & 0.620 & FantasyID only \\
    TruFor baseline$^\dagger$       & 0.807 & 0.590 & 876K pre-training images \\
    \midrule
