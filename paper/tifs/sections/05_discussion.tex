...
zero when the segmentation head is removed (\texttt{cls\_only}: $0.002 \pm 0.008$
...
picture: \texttt{seg\_only} achieves a Dice score of $0.867 \pm 0.023$ that is
...
The comparison between Multitask and \texttt{unweighted\_losses} isolates the
...
\begin{enumerate}
  \item \textit{Eliminates the manual weight choice.} Scalar selection requires
  ...
  \item \textit{Reveals the trade-off landscape.} The 500-point objective space
  ...
  \item \textit{Is consistent with the evaluation protocol.} The DeepID 2025
...
Numerically, the significant difference against \texttt{unweighted\_losses} in the
...
\paragraph{Single dataset and limited cross-domain generalization.}
...
scanner artifacts) remains an open question and a known limitation of all systems
trained on a single synthetic source. Extension to additional datasets such as
SIDTD~\cite{sidtd2024} would substantially strengthen the external validity
of the conclusions.

\paragraph{Architecture.}
The DocVerify encoder is a custom lightweight CNN trained from scratch. Transformer-based
encoders~\cite{dosovitskiy2021vit} and models pre-trained on large-scale vision datasets
