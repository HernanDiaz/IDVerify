...
Windows~11, equipped with an AMD Ryzen~5 3400G (3.70\,GHz) and 32\,GB DDR4-3200 RAM,
using PyTorch 2.10.0 with CUDA 13.0 and Optuna~3.x. Training uses a batch size of
64; no data augmentation is applied --- images are resized to $224\!\times\!224$
pixels and normalised to $[0,1]$ prior to caching. The full nested cross-validation
pipeline (including 2\,500 HPO trials and 10 final models) required approximately
59 hours of wall-clock time. The scalar experiment
...
\begin{itemize}
  \item \textit{Multitask} (proposed): joint detection + localization with
        Pareto-optimized $\lambda_{\text{mask}}$.
  \item \texttt{cls\_only}: classification head only; no segmentation head or
        segmentation loss.
  \item \texttt{seg\_only}: segmentation head only; classification is performed
        by thresholding the mean mask probability.
  \item \texttt{unweighted\_losses}: full multitask architecture with equal
...
essential: \texttt{cls\_only} produces near-zero Dice ($0.002 \pm 0.008$),
...
\texttt{seg\_only} achieves competitive Dice ($0.867$) but its PR-AUC drops to
...
measurable benefit over equal weights: \texttt{unweighted\_losses} matches
...
\texttt{by\_prauc} (maximize PR-AUC) and \texttt{by\_dice} (maximize Dice).
